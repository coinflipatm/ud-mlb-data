# 8/31 Triple Play board v1 (built 10:30 AM ET, pre-lineup-post)
Slate: MIL@CHC, DET@MIN, ATH@TEX, CWS@HOU, BAL@COL, NYY@LAA, PHI@AZ. 3 entries, $3 Triple Play (6-man/6-rd, 36-pick queues).
Lines: Covers/ESPN ~9:44am. Tiers: A = BAL 5.78, COL 5.22, MIL 4.75, CHC 4.75 | B = NYY 4.49, MIN 4.45, HOU 4.42, TEX 4.34 | C = rest.
Slots for MIL/CHC/TEX/HOU/MIN/CWS/PHI/AZ/DET/LAA/ATH = modal slot last 10 starts (repo boxscores, thru 8/30), NOT posted cards. BAL/COL/NYY = FantasyData projected card. Q/out: Alonso, Basallo, Lombard. No-ADP (undraftable): Tovar, Wells, Veen, Lewis, Keith, Muncy, Heim + all dash-ADP names.

## Arms (2026, pitcher_logs thru 8/29; rank = K/9 then ML, legal = confirmed non-dog non-Coors)
1. deGrom TEX 10.36 K/9, 4.30 K/BB, 25 GS, -222  <- T1
2. Harrison MIL 10.29, 4.70, 21 GS, -110 (unused: Game B arm)
3. Bradley MIN 9.81, 2.68, 26 GS, -135  <- T2
4. Nola PHI 8.90, 3.12, 27 GS, -117 (4.66 ERA, 1.86 HR/9 - alt only)
5. Lambert HOU 8.05, 2.45, 23 GS, -131  <- T3 (same-side w/ HOU stack)
6. Rodriguez NYY 5.68, 1.23, 5 GS, -143 | 7. Holmes CHC 5.88, 2.20, pick'em
DEAD: Bradish/Gordon (Coors), Jump +188, Kay +111, Jobe +115, Urena +120, Pfaadt +105 (dogs). R1 exception NOT met (deGrom-Harrison gap 0.07 < 1.0) -> bats R1, deGrom R2-3.
No spot-starter bumps: min GS on slate = Jobe 4, Rodriguez 5 (both >2).

## Room blueprints (zero overlap, arms never repeat, blocked list grows each room)
T1 Coors: 3 BAL + 2 COL + deGrom. Both-dugouts basis: total 11, Gordon 5.48 ERA/1.99 HR9; Bradish 1.41 WHIP (borderline - noted).
T2: 3-4 MIL + 1-2 MIN + Bradley (same-side MIN bonus). CHC excluded from top queue (both-dugout ban: Harrison 2.59/Holmes 2.03 both good).
T3: 4 HOU + 1 TEX/CHC + Lambert (same-side). CWS bats banned on this room (Rule 7 vs Lambert).

## Queue T1 (36)
Henderson BAL3, Mayo BAL4*, deGrom P, Goodman COL4, Castro COL6, Beavers BAL6, Rumfield COL3, Moniak COL5, CES BAL5^, Holliday BAL1^, Cowser BAL8, McCarthy COL1, Norby COL8, Contreras MIL3, Chourio MIL1, Bauers MIL3, Yelich MIL5, Mitchell MIL7, Turang MIL1^, PCA CHC1, Bregman CHC4, Suzuki CHC2, Happ CHC5, Taylor CHC6, Alvarez HOU2, Pena HOU1, Paredes HOU3, Duran TEX5, Langford TEX3, Burger TEX6^, Bell MIN4, Keaschall MIN1, Rice NYY2, Neto LAA1, Harper PHI3, Moreno AZ3.
*Mayo slot = last-10 modal; actual post-Alonso/Basallo card TBD. ^floor <=40% last 10 - avoid unless forced.

## Queue T2 (36) - cross off T1 picks first
Contreras, Chourio, Bradley P, Bauers, Yelich, Mitchell, Turang^, Hamilton MIL9, Bell, Keaschall, Clemens MIN5, Jeffers MIN3, Duran, Langford, Seager TEX2, Foscue TEX1, Freeman TEX8, [unused Coors names in T1 order], Rice, Bellinger NYY4, Jones NYY6, PCA, Bregman, Suzuki, Happ, Neto, Trout LAA2, Harper, Schwarber PHI1, Moreno, Tawa AZ6, Perdomo AZ4, Carroll AZ2, Alvarez, Pena, Paredes.

## Queue T3 (36) - cross off T1+T2 picks; NO CWS
Alvarez, Pena, Lambert P, Paredes, Altuve HOU4, Y.Diaz HOU7, Walker HOU6^, Varsho HOU5^, Duran, Langford, Burger^, Seager, Foscue, [unused CHC block], [unused Coors block], Rice, Bellinger, Jones, Grisham NYY1, Bell, Keaschall, Clemens, Neto, Trout, Harper, Schwarber, Moreno, Tawa, Carroll, Perdomo.

## Floor-guard reds (<=40% of last 10 starts >=7 pts)
Murakami CWS 10% (ADP 18.7 trap), Cam Smith 11%, Kepler 17%, Nimmo 20%, Caballero 20%, Marsh 20%, Larnach 25%, Taveras 30%, Carrigg 30%, Moniak 30%, Burger 30%, Walker 30%, Ballesteros 30%, CES 40%, Holliday 40%, Turang 40%, Hoerner 40%, Varsho 40%, Bellinger 33%, Nola-target note: AZ C-tier fliers live (1.86 HR/9).

## v1.1 addendum: full 36 queues with arm ladders (autopick-safe)
R1: Henderson, Mayo, deGrom, Goodman, Castro, Beavers, Rumfield, Moniak, CES, Holliday, Cowser, McCarthy, HARRISON, Norby, Contreras, Chourio, Bauers, Yelich, Mitchell, Turang, PCA, Bregman, BRADLEY, Suzuki, Happ, Taylor, Alvarez, Pena, Paredes, Duran, NOLA, Langford, Bell, Keaschall, Rice, Neto.
R2: Contreras, Chourio, BRADLEY, Bauers, Yelich, Mitchell, Bell, Keaschall, Clemens, Jeffers, Turang, Hamilton, NOLA, Duran, Langford, Seager, Foscue, Freeman, Goodman, Castro, Rumfield, Moniak, HARRISON, Henderson, Mayo, Beavers, Rice, Bellinger, Jones, PCA, Bregman, RODRIGUEZ, Suzuki, Happ, Schwarber, Harper. (No LAA bats, no Holmes.)
R3: Alvarez, Pena, LAMBERT, Paredes, Altuve, Y.Diaz, Walker, Varsho, Duran, Langford, Seager, Foscue, NOLA, PCA, Bregman, Suzuki, Happ, Taylor, Rice, Grisham, Jones, Bellinger, HARRISON, Henderson, Mayo, Goodman, Castro, Beavers, Rumfield, BRADLEY, Neto, Trout, Harper, Moreno, Tawa, Carroll. (No CWS, no Rodriguez.)
Pre-declared arm-conflict crosses: Harrison=arm -> cross CHC bats. Nola=arm -> cross AZ bats. Rodriguez=arm -> cross LAA bats. Holmes never queued where MIL bats are. Cross-off carried room to room; repeats only via Rule 9 exception.

## v1.2: Triple Play missed (filled). Pivot to Double Play x2 ($25, 96 cap, 16 rooms of 6, $500/1st, pays 16).
Two rooms only: R1 Coors+deGrom (unchanged), R2 MIL/MIN+Bradley (revised - absorbs HOU names freed by cutting R3; tier order A-leftovers before B).
R2 revised 36: Contreras, Chourio, BRADLEY, Bauers, Yelich, Mitchell, Bell, Keaschall, Clemens, Jeffers, Turang, Hamilton, NOLA, Goodman, Castro, Rumfield, Moniak, Henderson, Mayo, Beavers, LAMBERT, Alvarez, Pena, Paredes, Altuve, Duran, Langford, Seager, Foscue, HARRISON, Rice, Bellinger, Jones, Grisham, Schwarber, Harper. (No LAA, no CWS-vs-Lambert issue: if LAMBERT becomes the arm, cross nothing - no CWS queued; if HARRISON becomes arm, no CHC queued here at all now; NOLA arm -> no AZ queued. Clean.)
Timing: Double Play 14.6% full at 9:48a - draft AFTER lineups post (~4:30-5:30p) on board v2 with confirmed slots; if fill crosses ~75% earlier, enter on v1.

## Tickets as drafted (pre-score; scores fill at final)
T1 Double Play $25, slot 6: NOLA | Henderson BAL3?, Mayo BAL4?, Goodman COL3 | Beavers BAL5, Moniak COL6. 3 BAL + 2 COL + arm from PHI@AZ. Queue ran as designed (deGrom went at 5).
T2 Double Play $25, slot 6: BRADISH (autopick, Coors - Rule 1; Castro vs own P - Rule 7) | Contreras MIL4, Castro COL5 | Chourio MIL2, Yelich MIL5, Bell MIN5. O-30.
M1 Moonshot $7, slot 5: HOLMES | Alonso BAL (Q, out - swap unconfirmed), Contreras MIL4 (vs own P - Rule 7), Pena HOU1 | Crow-Armstrong CHC1, Beavers BAL5. O-31.
M2 Moonshot $7: HARRISON | Goodman COL3, Holliday BAL1, Rumfield COL4 | Alvarez HOU2, Beavers BAL5. Clean.
First pitch 7:40p ET (MIL@CHC, DET@MIN); BAL@COL 8:40p; last game PHI@AZ 9:40p. Pending: Exposure-page screenshot for O tags / Alonso swap (Rule 18).
